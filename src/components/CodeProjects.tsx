...
export default function CodeProject () {
    return (
        <div id="code" className="scroll-mt-28">
            <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Code Projects:</h1>
            <div className="p-1 mt-0 mb-0 md:mt-2 md:mb-2 mx-auto bg-zinc-900 bg-opacity-60">
                {codeProjects.map((code) => {
                    return (
                        <a href={code.codeUrl.toString()} target="_blank" className="group mb-4 hover:shadow-lg rounded-xl transition duration-200 relative border border-slate-100 dark:border-slate-400 w-full h-full block">
                            <div key={code.codeUrl.toString()} className="flex flex-col items-start rounded p-4 relative h-full">
                                <h4 className="text-xl font-bold tracking-tight text-gray-900 dark:text-gray-100">{code.title}</h4>
                                        {/* <a href={project.codeUrl.toString()} target="_blank">Github</a> */}
                                        <p className="leading-6 pt-4 text-gray-700 dark:text-gray-300">{code.description}</p>
